...
csasdown::csas_table(dat, digits=2, booktabs = T, caption = "Résumé des moyennes de distribution a posteriori et des quantiles (5 %, 50 % et 95 %) pour les paramètres du modèle de stock-recrutement et le PRI fondé sur Ggén selon l’ajustement du modèle de Ricker\\_aprioriCap. Le paramètre « adjProd » est la productivité effective, $\\alpha$', de l’équation 15 dans le document principal.")
 
```

```{r coho-Ricker-alphaPosts, fig.cap="Distributions a posteriori des paramètres de Ricker $\\alpha$, par UC, obtenues en ajustant le modèle stock-recrutement de base de Ricker.", warning=FALSE, echo=FALSE, out.width = '90%',  fig.align="center"}
knitr::include_graphics("figure/coho-alphaPosts.png")
```
<!--Posterior distributions for Ricker $\\alpha$ parameters, by CU, obtained from fitting the base Ricker stock-recruitment model-->

\newpage

```{r coho-Ricker-adjProdPosts, fig.cap="Distributions a posteriori des paramètres de productivité effective ($\\alpha$'), par UC, obtenues en ajustant le modèle stock-recrutement de base de Ricker.", warning=FALSE, echo=FALSE, out.width = '90%',  fig.align="center"}
knitr::include_graphics("figure/coho-adjProdPosts.png")
```

\newpage

```{r coho-priorCap-alphaPosts, fig.cap="Distributions a posteriori des paramètres de Ricker $\\alpha$, par UC, obtenues en ajustant le modèle stock-recrutement de Ricker\\_aprioriCap.", warning=FALSE, echo=FALSE, out.width = '90%',  fig.align="center"}
knitr::include_graphics("figure/coho-alphaPosts-priorCap.png")
```

\newpage

```{r coho-priorCap-adjProdPosts, fig.cap="Distributions a posteriori des paramètres de productivité effective ($\\alpha$'), par UC, obtenues en ajustant le modèle stock-recrutement de Ricker\\_aprioriCap.", warning=FALSE, echo=FALSE, out.width = '90%',  fig.align="center"}
...
## L’abondance des géniteurs projetée utilisées pour des méthodes du PRL de projection

```{r coho-CUProjections-Ricker, fig.cap="L’abondance des géniteurs projetée, pour chacun des cinq cohos du Fraser intérieur UCs, utilisé pour développer la PRL de projection sous le modèle de base Ricker. La ligne continue montre la médiane abondance des géniteurs dans une année de projection tandis que l'ombrage gris montre les 10e et 90e centiles d'abondance des géniteurs.", warning=FALSE, echo=FALSE, out.width = '80%',  fig.align="center"}
...
```{r coho-CUProjections-Ricker-priorCap, fig.cap="L’abondance des géniteurs projetée, pour chacun des cinq cohos du Fraser intérieur UCs, utilisé pour développer la PRL de projection sous le modèle de Ricker\\_aprioriCap. La ligne continue montre la médiane abondance des géniteurs dans une année de projection tandis que l'ombrage gris montre les 10e et 90e centiles d'abondance des géniteurs.", warning=FALSE, echo=FALSE, out.width = '80%',  fig.align="center"}
