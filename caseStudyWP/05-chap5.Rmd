...
& 1.5-2.5 & *50th (lower and upper) & Further evaluation required & Further evaluation required \\               \hline
\end{tabular}
(\#tab:holt-tab6)
\end{table}
...
<!--LW attempt to recreate Holt et al. 2018 Table 6 
...
{r holt_tab6b, echo=TRUE}
# read.csv("data/holt_table6.csv", check.names = FALSE) %>% 
...
#  mutate(`Overarching goal` = ifelse(is.na(V1),
#                                     "",
#                                     V1)) %>% 
 csas_table(format = "latex",
             ...
             caption = ifelse(french,
                              "French goes here",
                              "Selected percentile-based lower and upper benchmarks identified to be similar or higher in value than stock-recruitment based benchmarks under the WSP, along gradients in productivity (recruits/spawner) and average harvest rates. * denotes the low-productivity scenario where lower and upper Ricker-based benchmarks are very close to one another, resulting in lower and upper percentile-based benchmarks that are the same. From Holt et al. 2018.")) %>%
  ...
  kableExtra::row_spec(4, extra_latex_after = "\\cmidrule(l){2-2}") %>% 
  ...
  kableExtra::column_spec(column=3, width = "20em") %>%
  kableExtra::column_spec(column=4, width = "20em") %>%
  kableExtra::column_spec(column=5, width = "20em")
