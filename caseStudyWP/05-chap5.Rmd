...
Due to poor logistic model fits using the entire 1953-2018 time series for both $S_{gen}$ and percentile benchmarks, we did not conduct a full retrospective analyses for this SMU. The underlying data characteristics that lead to poor logistic model fits are highlighted in the results section below.
...
Using this method, 2 out of 5 CUs with data in the most recent year of data (2018) would be above their lower benchmark (amber or green zone) and 3 would be below (red zone. Over the time series, status for Howe Sound-Burrard Inlet and Georgia Strait has improved, while status in other CUs has declined or swithced from green to red several times. 
...
knitr::include_graphics('figure/fig_status_by_CUscenario_5.png')
```

**Comparison of Percentile and Multi-Dimensional Methods**

We compared results of LRP status for percentile and decision tree methods, using different data sets (Table \@ref(tab:LRP-scenarios)). The status of component CUs based on percentiles used in this comparision is shown in Figure \@ref(fig:chum-perc-status-static). 
...
Diagnostics for the logistic regression for percentile benchmarks and proportion of CUs above their lower benchmark = 0.5 are summarized below. *\textcolor{cyan}{LW: need input on how much detail to include here}*
...
hit ration = 0.7. Ratio of correct classification. Moderately high. 
...
Useful for mixture of data qualities/types/BM e.g., some CUs didn't have appropriate RelAbdundance benchmarks. 

Caveats with using muliti-dimensional approach
