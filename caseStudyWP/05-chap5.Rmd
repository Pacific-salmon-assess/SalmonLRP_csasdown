...
The data we used had years where not all streams were counted. When a CU had some streams that were counted and some that were not in a given year, we infilled the missing streams at the stream level (Figure \@ref(fig:chum-escapement-infill)). When there were no counts of spawners for a CU in a given year, we infilled at the CU level for that year and CU. We had to infill by CU to get total spawners to use for the run reconstruction, but we did not use CUs with CU-level infilling in the retrospective analysis of LRPs because the infilling procedure assumes that escapement is correlated between CUs in a given year. 
...
2. Infill wild escapement by CU, in order to get a time series of wild escapement to use in retrospective analyses of LRPs.

##### Infilling by stream

This applies to CUs and years when there were counts in some streams in the CU in a given year and some streams without counts. For each stream, the geometric mean of escapement over all years was calculated as the stream's average escapement. The total average escapement for each CU in each year was the sum of the average escapements from all streams. Then a proportion of monitored escapement in each year was the sum of average escapement of all streams with counts in a year divided by the sum of the average escapements for all streams (counted and uncounted) in that CU. The infilled escapement for a CU in given year was the sum of the observed escapements for that CU and year divided by the proportion of the monitored escapement for that CU and year. 

Infilling by stream typically made up a small proportion of the total escapement for each CU, with the exception of Howe Sound-Burrard Inlet (Figure \@ref:fig(chum-escapement-infill)). This was partly due to increasing escapements in the Cheakamus River and Indian River since 2000. 

This method assumes that escapement among streams is correlated, which is not always the case *\textcolor{cyan}{LW: can have figure in appendix or quote correlation values}*. 

##### Infilling by CU

If there were no counts of any streams in a CU in a given year, a second round of infilling was done with the data set that had already been infilled by stream. This was the case for two CUs: Upper Knight (22 years: 1979-1980, 1982, 1984, 1989, 1991, 1996, 2004-2018) and Bute Inlet (13 years: 2005-2006, 2008-2018). 
...
Due to poor logistic model fits using both $S_{gen}$ and percentile benchmarks, we did not conduct retrospective analyses for this SMU. The underlying data characteristics that lead to poor logistic model fits are highlighted in the results section below.
...
Based on this previous work, we used 25% of spawner abundance as a benchmark for Georgia Strait and Howe Sound Burrard Inlet, 50% for Loughborough, Northeast Vancouver Island, Upper Knight, and the multidimensional approach for Bute Inlet and Southern Coastal Streams.*\textcolor{cyan}{LW: may need to revise the last three based on multi-dimensional methods}*
...
The State of the Salmon program (Fisheries and Oceans Canada) is developing another method of assessing the status of CUs (ref Grant & ___). This method uses a decision tree to evaluate status based on data type, quality, abundance, and trends \@ref(fig:decision-tree) *\textcolor{cyan}{probably more here when we get the background methods}*. This work is based on Wild Salmon Policy status assessments for Fraser River sockeye (**REF**), Interior Fraser coho (**REF**), and Southern BC Chinook (**REF**). As this methodology is developed, it presents an alternative method of asssessing CU status that can be more flexible to variable quality of data, trends, etc.
