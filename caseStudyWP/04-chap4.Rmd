...
To derive habitat-based benchmarks, watershed areas were updated for WCVI Chinook using methods described in @parken_habitat-based_2006 by identifying 3rd order watershed areas that contain spawning habitat and omitting areas above obstacles to fish passage from the 
...
Our approach to estimating benchmarks differs from that of @parken_habitat-based_2006, because we derived productivity independently where @parken_habitat-based_2006 estimated both $S_{MSY}$ and $S_{REP}$ from the watershed-area model thereby inferring mean estimates of productivity which were deemed unrealistically high for WCVI Chinook (>7 recruits/spawner). 
...
We evaluated status of WCVI Chinook using LRPs estimated using the proportion of inlets above $S_{gen}$, projection-based LRPs, and the WSP integrated assessment (2014 only, DFO 2016) (Figure \@ref(fig:chinook-retro)). All methods indicate this SMU being below its LRP for years where data are available.
