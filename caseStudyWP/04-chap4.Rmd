...
\begin{table}[ht]
\centering
\caption{Overview of WCVI Chinook Stock Management Unit. Italics represent escapement indicators with average PNI values $\geqslant$ 0.5 and are excluded from analyses. The inlets, San Juan and Nitinat do not contain escapement indicator populations with PNI $<$ 0.5 and are not included in these analyses. }
...
We further considered a proportional LRP based on CU statuses derived from a previously published WSP integrated assessment [status in 2014 only, @dfoIntegratedBiologicalStatus2016].
...
All methods indicate this SMU being below its LRP for years where data are available. We use the same nomenclature as for Interior Fraser River Coho Salmon, where 'Prop' indicates proportional LRPs, 'Abund' indicates aggregate abundance LRPs, 'Scanner' indicates CU-level assessments derived from multidimensional approach within the the Pacific Salmon Status Scanner, 'Sgen' indicates CU assessments derived on a single metric, (e.g., abundances relative to the lower benchmark S~gen~), and 'Proj' indicates projection LRPs. 
