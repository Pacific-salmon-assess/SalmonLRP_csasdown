...
# Ask Luke to change San Juan River to San Juan, and Nitinat Lake to Nitinat
```
...
[Provincial Obstacles to Fish Passage database](https://catalogue.data.gov.bc.ca/dataset/provincial-obstacles-to-fish-passage) (Appendix X). Only watershed areas of indicator stocks were included in the current analyses, which are those stocks with the relatively complete time-series and consistent observation methodology (Table \@ref(tab:chinook-WA)). In future analyses, watershed areas of all known spawning populations could be included (omitting areas above obstacles to fish passage) to derive habitat-based benchmarks on an absolute abundance scale. These benchmarks could be compared against total abundance to each inlet. This approach was not used as a base case because of large uncertainties in abundances of non-indicator stocks.
...
csasdown::csas_table(dat, booktabs = T, caption = "Watershed areas for indicator stocks, km\\textsuperscript{2}.")
...
### PROPORTION CUs
$S_{REP}$ values were derived from the watershed area model (Appendix X). The lower benchmark, spawner abundances required to achieve $S_{MSY}$ within one generation without fishing under equilibrium conditions, $S_{gen}$, was derived by optimizing the Ricker equation with recruitment set to $S_{MSY}$,
...
<!--CH comment: Revise table to include horizontal line after individual stocks, or to exclud them entirerly, and just show inlets.... (this is probably best, then explain how I summed WAs within inlets-->
...
csasdown::csas_table(bench, booktabs = T, caption = "Benchmarks and boostrapped generated 95% confidence intervals for individual indicator stocks and aggregated to inlets.") %>% kableExtra::row_spec(20, hline_after=TRUE)
