...
The average generation time is consistent across all three CUs, and they exhibit the ocean-type life-history variant and fall run timing.  Absolute abundances are not available for these CUs [@brown2020SummaryAbundance2020]. 
...
CU status was derived from the proportion of component inlets above lower benchmarks. Serious harm is expected to occur when any one inlet within each of the 3 CUs drops below its lower benchmark. Here we have assumed the status of the one escapement indicator stock in the West Vancouver Island-North CU (Marble River) is representative of the CU. However, further consideration of this assumption is warranted with local expertise.

## LRP ESTIMATION: PROPORTION OF CUS

### Methods

The LRP on the proportion of CUs was identified as all 3 CUs containing inlets with current statuses exclusively above Red status.  Because inlets are nested within CUs, this LRP accounts for the distribution of spawning among inlets within CUs. If any CU contained an inlet with Red status, the LRP was considered breached. Status of component inlets was derived from the multidimensional approach used within the Pacific Salmon Status Scanner or on a single metric of spawner abundances relative to the lower benchmark, S~gen~. 

We further considered a proportional LRP based on CU statuses derived from a previously published WSP integrated assessment [status in 2014 only, @dfoIntegratedBiologicalStatus2016, Figure \@ref(fig:chinook-retro)].

### Results


Using the multidimensional approach implemented within the Salmon Scanner, for this SMU, status of component inlets can be inferred from spawner abundances against abundance-based lower benchmarks (Figure \@ref(fig:chinook-InletTimeSeries)). This is because time-series of abundances for WCVI Chinook are not absolute (only indicator stocks are monitored consistently) and relative-abundance benchmarks can be identified (S~gen~ and S~MSY~).  In the most recent year with data, 2020, 4 of 5 inlets were above their abundance-based lower benchmark, S~gen~ (Figure \@ref(fig:chinook-InletTimeSeries)). Therefore, 2 of 3 CUs contained inlets with current statuses exclusively above their lower benchmarks. One CU, Southern Vancouver Island, contains an inlet, Clayoquot, with status that has been consistently below its lower benchmark throughout the available time-series. Therefore this SMU falls below the LRP of 100% of CUs above Red status.

The status on a single metric of spawner abundances relative to benchmarks is therefore the same as that based on the multidimensional approach within the Salmon Scanner. 
...
- The projection LRPs should further be evaluated for sensitivity to key structural assumptions, such as the stock-recruitment model and presence and magnitude of depensation among other assumptions. This evaluation is especially important for this SMU because of the lack of peer-reviewed stock assessments that include stock-recruitment relationships. We based our assumption of a Ricker form of the stock-recruitment relationship based on its application to other Chinook stocks in BC [parkenHabitatbasedMethodsEstimate2006], but this form may not represent the high observed variability and declining productivity that are now common for Chinook salmon. 

-In the development of proportional and projection LRPs, inlets were chosen as the spatial scale of biodiversity required for the sustainability for the SMU. In future analyses, alternative assumptions could be considered, including LRPs derived to maintain diversity at the CU scale by projecting CU-level abundances.  
...
- Further evaluation of the influence of hatcheries on spawner abundances and model assumptions, including the form of the stock-recruitment relationship, would be required prior to providing aggregate abundance LRPs for this SMU. 
