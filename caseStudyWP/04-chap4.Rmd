...
CU status was derived from the proportion of component inlets above lower benchmarks. Serious harm is expected to occur when <!--KH: changed from 'was identified as'--> any one inlet within each of the 3 CUs drops below its lower benchmark. Here we have assumed the status of the one escapement indicator stock in the West Vancouver Island-North CU (Marble River) is representative of the CU. However, further consideration of this assumption is warranted with local expertise.
...
## LRP ESTIMATION: AGGREGATE ABUNDANCE LOGISTIC REGRESSIONLRPS
...
Candidate projection LRPs were compared against time-series of aggregate abundances observed for WCVI Chinook salmon (sum of indicator stocks with PNI > 0.5), showing that abundances are currently below these LRPs and have been near or below them over the available time-series (Figure \@ref(fig:chinook-statusProjLRP)).
...
We evaluated the status of WCVI Chinook using LRPs estimated using the proportion of CUs with all inlets above S~gen~ and projection LRPs, as well as the previously published WSP integrated assessment [status in 2014 only, @dfoIntegratedBiologicalStatus2016, Figure \@ref(fig:chinook-retro)]. All methods indicate this SMU being below its LRP for years where data are available. We use the same nomenclature as for Interior Fraser River Coho Salmon, where 'Prop' indicates proportional LRPs, 'Abund' indicates aggregate abundance LRPs, 'Scanner' indicates CU-level assessments derived from the Pacific Salmon Status Scanner, 'Sgen' indicates CU assessments derived on a single metric, (e.g., abundances relative to the lower benchmark S~gen~), and 'Proj' indicates projection LRPs. For this SMU, the two approaches to CU assessments gave the same statuses, and are repeated in the plot for emphasis. 
