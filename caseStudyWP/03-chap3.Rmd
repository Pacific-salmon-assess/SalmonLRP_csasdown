...
\endhead
\hline
 Ricker Parameters ($\alpha$, $\beta$, $\gamma$,$\sigma$)  &  CU-specific (Appendix xxx) & Stock recruit parameters drawn from joint posterior from MCMC model fit to data from 1998 - 2020
\\\\

Smolt-to-adult marine survival rate distribution (all CUs) & Log-normal(-4.83, 1.21), bound between [-9.21, -3.32]
 & Estimated from time-series of annual marine survival rate estimates from brood years 1998 - 2016, with bounds set at lowest and highest observations (M. Arbeider, pers. comm) 
\\\\  

 Among-CU variability in marine survival coefficient $\gamma$  &  $\sigma_{\gamma}$ = 0 (all CUs the same) & Assumed value when fitting models. Varied between 0 and 0.09 in sensitivity analyses, where upper limit of 0.09 is the standard deviation of the estimated marginal posterior distribution for $\gamma$ in current analyses. 
\\\\

 Ave age proportions at maturity (ages 3, 4) &  MiddleFR, LThomp, SThomp = (0.86,0.14) , FRCanyon = (0.87, 0.13), NThomp = (0.88, 0.12) & Estimated from time-series of ppns of ages-at-maturity (M. Arbeider, pers. comm) 
 \\\\  

 Variability in age proportions (tau from multivariate logistic distribution)  & MiddleFR, NThomp, SThomp = 1.0, LThomp = 0.9, FRCanyon = 0.8 & Estimated from time-series of ppns of ages-at-maturity (M. Arbeider, pers. comm). Assumed variable over CUs and years. \\\\

 Average exploitation rate & 0.125 & Estimated from time-series of annual exploitation rate estimates from brood years 1998 - 2016. Varied in sensitivity analyses 0.05 - 0.35.
 \\\\

Interannual variability in exploitation rates & CV = 0.442  & Estimated from time-series of age-3 ER estimates from brood years 1998 - 2016. Assumed to be beta distributed, constrained between 0-1.
\\\\

Variability in exploitation rates among CUs & CV = 0.221 &  Assumed to be half of interannual variability, varied in a sensitivty analysis (0-0.442). Assumed to be beta distributed, constrained between 0-1.
\\\\ 

Initial abundances  & Initialized using spawner-recruit time series & Data provided by M. Arbeider (pers. comm)
\\\\
