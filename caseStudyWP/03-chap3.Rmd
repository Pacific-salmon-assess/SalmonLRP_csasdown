...
<!--TODO: change all reference of multidimensional tool to the Pacific Salmon Status Scanner and for short the Salmon Scanner-->
...
WSP CUs have been identified for Interior Fraser Coho based on genetics and geographic separation: Middle Fraser, Fraser Canyon, Lower Thompson, North Thompson, and South Thompson [@dfoWildSalmonPolicy2015; Figure \@ref(fig:coho-map)]. Previous work by the Interior Fraser Coho Recovery Team (IFCRT) identified 11 subpopulations nested within the five CUs, and developed recovery objectives based on maintaining abundance <!--BC:Is there a better way to say this? abundance above a threshold? Self-sustaining populations? --> in each of these smaller subpopulation units [@ifcrtinteriorfrasercohorecoveryteamConservationStrategyCoho2006; Table \@ref(tab:cohoCU2SP)]. The delineation of subpopulations was based on several factors, including the presence of natural barriers, the influence of large lakes on downstream discharge and thermal regimes, observations of spawner aggregations under differing discharge conditions, and genetic differentiation. The 11 subpopulations are described in detail by the @ifcrtinteriorfrasercohorecoveryteamConservationStrategyCoho2006. 
...
<!--CW: I started translating the discussion points into paragraphs below -->

The Interior Fraser Coho SMU is considered a data-rich SMU as it has full stock-recruits time series for all five CUs within the SMU, which allowed for the estimation of abundance-based benchmarks (S\textsubscript{gen}). When considering the proportion-based LRPs, comparisons between single metric benchmark (S\textsubscript{gen}) and the Pacific Salmon Status Scanner yielded similar results. This occurs because the Pacific Salmon Status Scanner relies on S\textsubscript{gen} benchmarks when those are available. Exceptions occur if the estimated S\textsubscript{gen} are below the absolute threshold of 1500 spawners, as ocurred for the Frase Canyon CU. 

This Interior Fraser Coho SMU has a long history of using aggregate abundance-based recovery targets and fisheries reference points <!-- include citation --> that were developed based on an underlying relationship between aggregate abundance and the distribution of abundance among sub-populations and CUs <!-- include citation -->. As a result, the Interior Frase Coho SMU was well suited to looking at the application of aggregate abundance-based LRPs, and comparing them to the preferred proportional approach.


Our results highlight the complexities in predicting how well aggregate abundance-based LRPs will do at tracking proportion-based LRPs. Comparisons between the abundance-based methods and the proportional methods yielded mixed results. Status was generally consistent for the scenarios in which the Ricker_priorCap model was used, but diverged more for the Ricker and IFCRT methods. The proportion-based methods tend to get triggered more often in years 2000-2001 and 2015-2017, when abundance of individual CU's are low. Whereas the Aggregate abundance-based methods tend to get triggered ore often in years 2006-2007, the lowest aggregate abundance generational averages for the time-series. 


Most methods evaluated for the Interior Frase Coho case study relied on the evaluation of CU status relative to S\textsubscript{gen}. As a result, the method used to estimate S\textsubscript{gen} had a large impact on results. We used two different approaches for estimating S\textsubscript{gen}: a Ricker model ( base Ricker) and a ricker mode with an informative prior on S~REP~ (Ricker_priorCap). S\textsubscript{gen} estimates were higher for the Ricker_priorCap model compared to the base Ricker model, which meant that LRPs were more frequently triggered under this formulation. This pattern was observed for all four methods that relied on the Ricker_priorCap S\textsubscript{gen} estimates.

 We considered alternative Ricker models for Interior Fraser Coho  to represent different assumptions regarding productivity trends in the stock. This approach has also been used in previous analyses for this SMU. @arbeiderInteriorFraserCoho2020 used a model averaging approach with three SR models equally weighted when assessing recovery potential for the Interior Fraser Coho SMU (the base Ricker and Ricker_priorCap models we used, as well as a third depensatory mortality version that we did not consider). @kormanEvaluationFrameworkAssessing2019 also considered multimple Ricker model formulations when estimating reference points for the Interior Fraser Coho SMU. They opted for choosing a preferred model, the base Ricker, instead of using model averaging approaches. 


Implementation of LRPs when considerable uncertainty in underlying model structure exists will require methods to integrate estimates of LRP status over alternative model structures.  We demonstrate one approach when using projection-based estimates of aggregate abundance-based LRPs, in which we combine projections under each SR model scenario before calculating the LRP. This approach is basically a model averaging approach in which both scenarios are equally weighted.  However, other methods of assigning weights among model are possible. <!-- For example .... (TO DO: reference model selection / averaging papers from Carrie).  --> In other cases, such at the proportional approach using WSP assessments or the Pacific Salmon Status Scanner, uncertainty in model structure may require integration at earlier stages (i.e. when estimating biological benchmarks) or integration based on expert opinion.  For example, during the expert workshops that will be used to confirm Pacific Salmon Status Scanner status estimates, participants could be given two sets of status results based on two different Sgen models, and then be asked to converge on a single status estimate. <!--(can we pers. comm. Sue Grant as a reference for this process?). CW: I am not sure we want to give this example, I think it can be controversial and lead us on a off topic discussion --> 43
