...
@arbeiderInteriorFraserCoho2020 and @kormanEvaluationFrameworkAssessing2019 set $\mu_{S_{REP}}$ at 1.5 times the S\textsubscript{REP} value estimated from the base model fit without a prior on S\textsubscript{REP}. For our integrated Sgen-LRP model fits (described in Section \@ref(logistic)), we found that we needed to constrain $\mu_{SREP}$ at no more than 1.4 times the S\textsubscript{REP} value to achieve model convergence, so we used the 1.4 times expansion instead.  We set $\sigma_{SREP}$ at $\sqrt{2} * 1000 = 1414$ spawners, which is the same value used by @arbeiderInteriorFraserCoho2020. Note that the "$* 1000$" term is used to correct for scaling spawner abundance by 1/1000 when fitting models. The effect of adding the prior on $S_{REP}$ when fitting individual models to available data is shown in Figure \@ref(fig:coho-SR-fit).  
