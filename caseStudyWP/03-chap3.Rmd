...
Comparison of SMU status estimates over time for all LRP estimation methods that used S\textsubscript{gen} estimates from the base Ricker model (first group of four methods in Figure \@ref(fig:coho-statusPlot-withBars)), showed that the LRP was only breached in years 2000, 2015-2017 under the Prop: Scanner-Ricker method. The other three Ricker methods estimated status to remain above the LRP in those years. The other three methods are similar in that they all assess CU status based on a direct comparison of CU-level abundance to S\textsubscript{gen}.  In contrast, the decision tree for the Pacific Salmon Status Scanner includes a step in which CU status is designated as 'red' anytime the generational mean spawning abundance drops below 1500 spawners (Figure \@ref(fig:decision-tree)).  Because estimated S\textsubscript{gen} is less than 1500 spawners for the Fraser Canyon CU, it is possible for the criteria of <1500 spawners in a CU to be breached even though abundance remains above S\textsubscript{gen}.  This situation occurs for the Fraser Canyon CU in 2015-2017. Therefore, LRP methods that use the Pacific Salmon Status Scanner to characterize CU status can be more precautionary than methods that rely on a single S\textsubscript{gen} benchmark.  

In the years 2005-2006, SMU status for both the 'Abund: Proj: Sgen-Ricker' and 'Abund: Logistic:Sgen-Ricker' methods fell below the LRP, while the Prop: Sgen-Ricker methods did not. Declines in aggregate SMU abundance in 2005-2006 were driven by declines in the four larger CUs (which, still remained above their individual S\textsubscript{gen} estimates). Declines in the lower abundance Fraser Canyon CU were not as drastic. As a result, while SMU-level aggregate abundance dropped below the abundance-based LRP, the Fraser Canyon CU that triggered 'red' status in 2015-2017 remained above 1500 spawners and did not trigger the Prop: Scanner-Ricker method. The aggregate abundance-based LRP from the 'Abund: Proj: Sgen-Ricker' estimation method was higher than that from the 'Abund: Logistic: Sgen-Ricker', so only the former method triggered an LRP breach. 

When the Ricker-priorCap model was used to estimate S\textsubscript{gen} instead of the base Ricker model, both S\textsubscript{gen} and LRP estimates were higher than under the base Ricker model formulation. This in turn resulted in more frequent LRP breaches when the Ricker-priorCap model was used (see second group of four methods in Figure ) compa\@ref(fig:coho-statusPlot-withBars)red to when the base Ricker model was used.  Among the 'priorCap' methods, status was most frequently estimated to be below the LRP when the 'Abund: Proj:Sgen-priorCap' method was used; for this method, the LRP was triggered in 12 out of the 21 years between 2000 and 2020. In comparison, the LRP was triggered in 9, 5, and 5 of the 21 years for the 'Prop: Scanner-priorCap', 'Prop: Sgen-priorCap' and 'Abund: Logistic:Sgen-priorCap' methods, respectively.    

Finally, SMU status was below the LRP in four (2006, 2015-2017) out of the 21 years between 2000 and 2020 for the 'Prop: IFCRT' method and remained above the LRP in all years under the 'Abund: Logistic: IFCRT method'. The 'Abund: Logistic: IFCRT method' produced the lowest LRP of all logistic methods (Figure \@ref(fig:coho-IM-logisticFit2020) and Table \@ref(tab:logisticLRPs2020)), therefore the LRP tends to get triggered less often.  
...
Our results highlight the complexities in predicting how well aggregate abundance-based LRPs will do at tracking proportion-based LRPs. Comparisons between the abundance-based methods and the proportional methods yielded mixed results. Status was generally consistent for the scenarios in which the Ricker_priorCap model was used, but diverged for the Ricker and IFCRT methods. When estimates of S\textsubscript{gen} were obtained using the Ricker_priorCap model, which produced higher S\textsubscript{gen} and LRP estimates than the Ricker and IFCRT methods, both types of proportion-based LRPs were less precautionary than aggregate abundance-based LRPs. This result was especially pronounced when aggregate abundance-based LRPs were estimated using the projection-based LRP approach.
