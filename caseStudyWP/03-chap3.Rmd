...
Comparison of SMU status estimates over time for all  LRP estimation methods that used $S_{gen}$ estimates from the base Ricker model (Prop: Scanner-Ricker, Abund: Logistic: Sgen-Ricker, and Abund: Proj: Sgen-Ricker; the first group of three methods in Figure \@ref(fig:coho-statusPlot-withBars)), showed that estimated LRP breaches coincided with low points in the aggregate abundance time series (2005 - 2006 and 2015-2017).  However, there were differences in which years different methods estimated SMU status below the LRP. Among the Sgen-Ricker CU options, the LRP was only breached in years 2015-2017 under the Prop: Scanner-Ricker method. Both of the aggregate abundance-based methods estimated status to remain above the LRP over this period. This result is due to the Prop method using the rapid multidimensional scanning tool to assess CU status rather than a direct comparison of CU-level abundance to $S_gen$. The decision tree for the rapid scanning tool includes a step in which CU status is designated as 'red' anytime the generational mean spawning abundance drops below 1500 spawners (Figure \@ref(fig:decision-tree)).  Because estimated $S_{gen}$ is less than 1500 spawners for the Fraser Canyon CU, it is possible for the criteria of <1500 spawners in a CU to be breached even though abundance remains above $S_{gen}$.  This situation occurs for this CU in 2015-2017. Therefore, LRP methods that use the rapid multi-dimensional scanning tool to characterize CU status can be more precautionary than methods that rely on a single $S_{gen}$ benchmark.   

In the years 2005-2006, SMU status estimates from the 'Abund: Proj: Sgen-Ricker' method fell below the LRP, while the other two Sgen-Ricker methods did not. The drop below the LRP in this year was due to declines in aggregate SMU abundance that were driven more by declines in the four larger CUs (which, still remained above their individual $S_{gen}$ estimates) than the smaller Fraser Canyon CU. As a result, while SMU-level aggregate abundance dropped below the abundance-based LRP, the Fraser Canyon CU that triggered 'red' status in 2015-2017 remained above 1500 spawners and did not trigger the Proportional LRP. The aggregate abundance-based LRP from the 'Abund: Proj: Sgen-Ricker' estimation method was higher than that from the 'Abund: Logistic: Sgen-Ricker', so only the former method tiggered an LRP breach. The 'Abund: Logistic: Sgen-Ricker' LRP method resulted in SMU status estimates above the LRP in all years.


Both $S_{gen}$ and LRP estimates were higher when the Ricker-priorCap model was used to estimate $S_{gen}$ instead of the base Ricker model, which resulted in more freqeunt LRP breaches for these methods in general (Prop: Scanner-priorCap, Abund: Logistic: Sgen-priorCap, and Abund: Proj: Sgen-priorCap; the second group of three methods in Figure \@ref(fig:coho-statusPlot-withBars)).



Comparison of SMU status estimates over time for all LRP estimation methods that used the Ricker-priorCap model showed that 
