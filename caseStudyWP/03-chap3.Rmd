...
<!--CH: This section on threats is very thorough. I wonder if it could be condensed to: "The dominant threats to this SMU are from forestry, forest fires, agriculture, and urban and rural development, including threats from effluents from agriculture and forestry and from linear development (i.e., the straightening and channelization of streams associated with agriculture and urban / rural development) [ @arbeiderInteriorFraserCoho2020]. While impacts from a given activity may be higher in some CUs that others, the interconnected threats stemming from any one activity, as well as cumulative impacts, mean that it is hard to isolate one CUs that experiences threats differently than another.  Furthermore,  extensive agricultural and linear development in the lower Fraser River, which all Interior Fraser Coho smolts migrate through, and a proportion of juveniles rear in, means that there is a possibility that all CUs are affected by threats stemming from agriculture and linear development."  --> 
<!--CH: If threats from ER are low, perhpas leave them out.-->

In an assessment of threats for Interior Fraser Coho completed as part of the 2018 Recovery Potential Assessment, @arbeiderInteriorFraserCoho2020 ranked a comprehensive set of threats as on a scale of high, medium, low, and unknown.  The highest ranking threat was modification to catchment surfaces, which was ranked as medium-high.  Examples of threats within this category included the removal of forest and vegetation and the creation of impervious surfaces resulting in modified hydrological regimes, while activities contributing to the modification of catchment surfaces included forestry, forest fires, agriculture, and urban and rural development.  The threat of effluents from agriculture and forestry, as well as the threat of linear development (i.e., the straightening and channelization of streams associated with agriculture and urban / rural development), were also ranked relatively high at medium risk. Forestry, agriculture, and linear development are widespread throughout the interior Fraser River watershed, with all CUs impacted by one or more of these [@arbeiderInteriorFraserCoho2020]. The Fraser Canyon, South Thompson, and Lower Thompson CUs have experienced the highest historic and ongoing forestry impacts, however, the Middle Fraser CU has had increased forestry in recent years due to pine beetle infestations. The three Thompson River CUs (North, South, and Lower Thompson) are most impacted by agricultural activities [@arbeiderInteriorFraserCoho2020], while linear development impacts are highest within the Middle Fraser and North Thompson CUs.  Development pressures linked to recreational use impact habitats within the South Thompson CU [@arbeiderInteriorFraserCoho2020]. Thus, while impacts from a given activity may be higher in some CUs that others, all CUs are vulnerable to these threats from one activity or another.  The interconnected threats stemming from any one activity, as well as cumulative impacts, mean that it is hard to isolate one CUs that experiences threats differently than another.  Furthermore,  extensive agricultural and linear development in the lower Fraser River, which all Interior Fraser Coho smolts migrate through, and a proportion of juveniles rear in, means that there is a possibility that all CUs are affected by threats stemming from agriculture and linear development.
...
Based on similarities in threats, environmental conditions, life history, and habitat capacity, we found no significant indicators that would prevent us from inferring CU status for one CU from neighboring CUs; especially when data from several other CUs are available to represent the missing CU. We do note however that the Fraser Canyon CU may be the most unique in terms of it having the smallest capacity and a more coastal dominated biogeoclimatic zone, so special consideration may be given to cases in which CU-level data is missing from Fraser Canyon.  
...
The first approach, which uses the Pacific Salmon Status Scanner tool developed by the State of the Salmon program (Section \@ref(rapidToolMethods)), is consistent with Canada's WSP and is recommended by Holt *et al.* (in review) as the method that should be used to estimate CU status when using the proportional LRP approach. The other two approaches are primarily used to develop aggregate abundance-based LRPs in this case study, as well as for a point of comparison with Pacific Salmon Status Scanner tool.

The second approach is based on comparing the current abundance of each CU to its CU-specific estimate of S\textsubscript{gen}, where CU status is considered poor when abundance drops below S\textsubscript{gen}. The value of S\textsubscript{gen} represents the number of spawners required to recover to S\textsubscript{MSY} (spawners maximum sustainable yield) within one generation, under equilibrium conditions in the absence of fishing [@holtIndicatorsStatusBenchmarks2009]. S\textsubscript{gen} is one of several benchmarks available for assigning multidimensional CU status in WSP Integrated Status Assessments; it represents a lower benchmark between red and amber status zones and was used as part of the 2014 Integrated Status Assessment for Interior Fraser Coho [@dfoWildSalmonPolicy2015].    
...
@arbeiderInteriorFraserCoho2020 and @kormanEvaluationFrameworkAssessing2019 set $\mu_{S_{REP}}$ at 1.5 times the S~REP~ value estimated from the base model fit without a prior on S~REP~. For our integrated Sgen-LRP model fits (described in Section \@ref(logistic)), we found that we needed to constrain $\mu_{SREP}$ at no more than 1.4 times the S~REP~ value to achieve model convergence, so we used the 1.4 times expansion instead.  We set $\sigma_{SREP}$ at $\sqrt{2} * 1000 = 1414$ spawners, which is the same value used by @arbeiderInteriorFraserCoho2020. Note that the "$* 1000$" term is used to correct for scaling spawner abundance by 1/1000 when fitting models. The effect of adding the prior on S~REP~ when fitting individual models to available data is shown in Figure \@ref(fig:coho-SR-fit).  
...
(ii)	A logistic regression model fit to aggregated data to estimate the LRP as the aggregate abundance that has historically been associated with a specified probability of all CUs being aboveS\textsubscript{gen} (Equations \@ref(eq:logistic) - \@ref(eq:logisticLRP))
