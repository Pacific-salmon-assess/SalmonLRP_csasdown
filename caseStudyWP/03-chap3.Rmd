...
<!--CH suggestion: "We recommend assessing status using the multi-dimensional approach when identifying proportional-based LRPs, and demonstrate two alternative approaches for comparison. In total, these approaches are:"--> <!--CH: I thought of putting this upfront to relay Sue's concerns about showing all methods on equal footing being confusing given her work.-->
We consider three alternative ways to characterize CU status when developing LRPs for Interior Fraser Coho:

1) Multi-dimensional rapid status assessment <!--CH: "Rapid Multidimenstional Scanner Tool (see Section \@ref(#rapidToolMethods)"- is the this official name?-->
...
<!--CH comment: I've suggested the text above to replace the first two sentences below. The third sentence could be revised to, "For aggregate-abundance based methods, both second and third approaches are applied". However, if we're applying the third approach outside of a fully integrated assessment (e.g., in logisitic-regression LRP), why can't we apply the first approach?. I think only the 2nd approach is used for projection-based LRPS, correct? -->
The first approach uses the multidimensional rapid scanning tool developed by the State of the Salmon program (Section \@ref(rapidToolMethods)). This method is used when developing proportion-based LRPs. In comparison, both the second and third approaches are primarily used when developing aggregate abundance-based LRPs. 
...
We evaluated aggregate abundance-based LRPs derived using logistic regressions with two of the Interior Fraser Coho benchmarks considered<!--Is it too much to consider the multidimensional approach too? It's hard to justify why we excluded it-->: $S_{gen}$ and the IFCRT-distributional target. Because two different spawner recuit models were used to estimate $S_{gen}$, we distinguish these models as 'Logistic:Sgen-Ricker' and 'Logistic:Sgen-priorCap' for the Ricker and Ricker_priorCap models, respectively. We use the label 'Logistic:IFCRT' to denote the case in which the IFCRT distributional target was used to develop an LRP.  See Section \@ref(logisticMethods) for an overview of the approach used to calculate aggregate abundance-based LRPs using logistic regression.
...
<!--
CW: Comments on logistic regression-based LRP based on multi-dimensinal status -- 
Not sure if we want to introduce this here. Or even if  we want to exclude the multidimensional status result. I think it may be unnecessary to include the plots of the poor fitting results (but these could be included in an appendix, if you think it would be beneficial, perhaps with the chum fits as well?). 


A third version of the logistic regression model, using the status assessments obtained with the multidimensional rapid status assessment tool was considered, but initial model evaluations led to the exclusion of that version of the logistic model. Two main reasons were the basis for this decision. First, the multidimensional rapid status assessment tool relies on generational mean (smoothed abundances) to assess status of individual CUs against benchmarks. For this reason, there was a weak/non-existent relationship between SMU status and the raw (not smoothed) abundances. Using the generational mean for the logistical regression fit introduced considerable autocorrelation in the abundance time series, which is a direct violation of the logistic regression assumptions.  Future explorations of aggregate abundance logitic regression-based LRPs based on the multidimensional rapid status assessment tool could account for the auto-correlation by including an AR(1) covariace structure. However, longer time-series and a wider spread of the data would be required to compensate for the additonal parameter in the model.    


Second, the multidimensional rapid status assessment tool includes criteria that are not continuously tied to the CU abundance. For example, CU status can be determined based on trends, which is not congruent with logistic regression goal of identifying underlying relationship between aggregate abundance and CU statuses. In addition, the multidimensional rapid status assessment tool can also rely on absolute abundance thresholds (e.g. generational mean should be above 1500 fish) that are not proportional to the size of a CU. These absolute benchmarks introduce a disconnect between the SMU abundance and status, even when there is high correlation between CUs. For example, a small CU may be above its specific Sgen but below the absolute threshold, this can lead to a red LRP status despite higher aggregate abundances. 
...
2015 was the first year in which the available IFC time series was long enough to estimate logistic regression-based LRPs using the Logistic:Sgen-priorCap and Logistic:IFCRT models. However, the Logistic:Sgen-priorCap model was unable to converge on an LRP estimate in 2018. The Logistic:Sgen-Ricker model required an additional two years of data before LRP estimates were first available in 2017. As a result, only four-six<!-- CH; 4-6 instead?--> years of retrospective analyses were available, depending on the model being considered. All three models showed some fluctuations in LRP estimates over time (Figure \@ref(fig:coho-retroLRPs)). The Logistic:IFCRT model tended to produce the most stable LRPs over time.  
...
Projection model outputs were used to estimate projection-based LRPs using the methods described on Section \@ref(projectedMethods). Forward projections were run for 30 years over 20,000 simulation trials.  The high number of simulation trials was required to stabilize LRP estimates given the binning of aggregate escapement in 200-fish intervals to identify LRPs based on probability thresholds.  <!--CH: maybe we shoould mention that initial trials over longer periods, 100 years, gave similar results-->
...
We assumed an average exploitation rate of 12.5% for all CUs in forward projections, <!--CH: based on ave historical values?-->with common interannual variability in exploitation rates due to shared fishery impacts among CUs each year. Interannual variability in exploitation rates was assumed to be beta distributed (constrained between 0 and 1), with the standard deviation of the beta distribution parameterized from estimated exploitation rates for 1998 - 2016 brood years. The corresponding coefficient of variation (CV) for interannual variability was 0.44.
...
In general, estimated LRP breaches coincided with low points in the aggregate abundance time series (2000, 2005 - 2006 and 2015-2017). However, there were differences among methods in the years that SMU status was estimated to be below the LRP, as well as a couple methods for which status was never estimated to be below the LRP (Abund: Logistic: Sgen-Ricker; Abund: Logistic: IFCRT).  

Comparison of SMU status estimates over time for all LRP estimation methods that used $S_{gen}$ estimates from the base Ricker model (Prop: Scanner-Ricker, Abund: Logistic: Sgen-Ricker, and Abund: Proj: Sgen-Ricker; the first group of three methods in Figure \@ref(fig:coho-statusPlot-withBars)), showed that the LRP was only breached in years 2015-2017 under the Prop: Scanner-Ricker method. Both of the aggregate abundance-based methods estimated status to remain above the LRP over this period. This result is due to the Prop method using the rapid multidimensional scanning tool to assess CU status rather than a direct comparison of CU-level abundance to $S_gen$.  The decision tree for the rapid scanning tool includes a step in which CU status is designated as 'red' anytime the generational mean spawning abundance drops below 1500 spawners (Figure \@ref(fig:decision-tree)).  Because estimated $S_{gen}$ is less than 1500 spawners for the Fraser Canyon CU, it is possible for the criteria of <1500 spawners in a CU to be breached even though abundance remains above $S_{gen}$.  This situation occurs for this CU in 2015-2017. Therefore, LRP methods that use the rapid multi-dimensional scanning tool to characterize CU status can be more precautionary than methods that rely on a single $S_{gen}$ benchmark.  <!--CH: perhaps we should have a version of this fig in the appendix, with Prop: SingleMetric-Ricker, in case reviewers are not happy with Multdimensional status?  Not sure...-->
...
When the Ricker-priorCap model was used to estimate $S_{gen}$ instead of the base Ricker model, both $S_{gen}$ and LRP estimates were higher than under the base Ricker model formulation. This in turn resulted in more frequent LRP breaches when the Ricker-priorCap model was used (Prop: Scanner-priorCap, Abund: Logistic: Sgen-priorCap, and Abund: Proj: Sgen-priorCap; the second group of three methods in Figure \@ref(fig:coho-statusPlot-withBars)) compared to when the base Ricker model was used.  Among the 'priorCap' methods, status was most frequently estimated to be below the LRP when the 'Abund: Proj:Sgen-priorCap' method was used; for this method, the LRP was triggered in 13 out of the 21 years between 2000 and 2020. In comparison, the LRP was triggered in 8 and 7 of the 21 years for the 'Abund: Proj:Sgen-Ricker' and 'Prop: Scanner-priorCap' methods, respectively.  LRP estimates based on the proportional LRPs estimated using the rapid multidimensional scanning tool were therefore the least precautionary of the thee priorCap methods.  
...
Discussion topic 2: How well do aggregate abundance-based LRPs perform retrospectively compared to proportional LRPs using the scanner tool?
...
- Our results highlight the complexities in predicting how well aggregate abundance-based LRPs will do at tracking proportional based ones.  For example, when estimates of Sgen were obtained using the base Ricker model, aggregate abundance-based LRPs were slightly less precautionary than using proprtional estimates using the scanner tool because the scanner tool used additional decision criteria based on minimum levels of absolute CU abundance. However, when estimates of Sgen were obtained using the Ricker_priorCap model, which produced higher estimates than the Ricker model, aggregate-based abundance LRPs were more precautionary than the proportional method; especially when aggregate abundance-based LRPs were estimated using the Projection-based LRP approach.  In this case, there were years in which aggregate abundance dropped below the LRP, but all CUs remained above red status using the mutli-dimensional tool.
...
<!--
Question: is there are strong need to estimate logistic regression-based LRPs that characterize CU status based on multi-dimensional status as part of this coho case study? In our current approach, we characterize CU status based simply on abundance relative to Sgen.
-->
