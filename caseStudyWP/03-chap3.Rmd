...
\begin{equation}
  S_{MSY,i} = S_{gen,i}e^(log(\alpha'_{i})- \beta_i \cdot S_{gen,i})
  (\#eq:Sgen)
\end{equation}
...
The failure of both the Logistic:Sgen-Ricker and Logistic:Sgen-priorCap models to converge in some years is a function of low sample sizes in the available data set, such that the removal of one or two CUs results in a failure of the logistic regression model to converge on a solution. Failure to converge usually occurs because 'successes' and 'failures' either overlap completely or don't overlap at all.  In the Logistic:Sgen-priorCap model the overlap was high and we found that removing one CU tended to increase overlap, while removing two CUs decreased overlap. For example, for the Logistic:Sgen-Ricker model, the South Thompson CU is an influential CU. Removing it from the characterization of 'successful' and 'failure' years, as well as the aggregate abundance used to fit the logistic regression, means that the model cannot converge on a solution because there was no overlap in aggregate abundance levels associated with 'successes' and 'failures', which is a requirement for logistic regression model fits. For the Logistic:Sgen-priorCap model, several CUs are influential due to higher S\textsubscript{gen} estimates compared to the  Logistic:Sgen-Ricker model. Because several different CUs contribute to 'failure' years, in which CU-level abundances drop below S\textsubscript{gen}, there is higher sensitivity to the removal of any one of these CUs.  
