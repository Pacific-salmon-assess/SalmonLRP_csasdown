...
We also evaluated the logistic regression-based LRP based on the results from the multidimensional rapid status assessment tool, however a couple points led to the exclusion of this method. 

First, the multidimensional rapid status assessment tool relies on generational mean to assess status of individual CU against benchmarks. We found that considering the generation mean as an indicator of aggregate abundance introduces considerable autocorrelation in the abundance time series which is a direct violation of the logistic regression assumptions. 
Second, the multidimensional rapid status assessment tool includes absolute abundance thresholds (e.g. generational mean should be above 1500 fish) that are not proportional to the size of a CU. These absolute benchmarks introduce a disconnect between the SMU abundance and CU status, even when there is high correlation between CUs. For example, a small CU may be above its specific Sgen but below the 1500 fish absolute threshold, this can lead to a red LRP status despite higher aggregate abundances. This disconnect was observed in preliminary exploration of logistic regression-based LRPs based on the multidimensional rapid status assessments results for the IF coho study case.   
