...
```{r coho-CU-timeseries, fig.cap="Escapement time series for five Interior Fraser Coho CUs. Solid red lines show estimates of Sgen from the IM model, while dashed red lines show estimates of Sgen from the IM.HiCap model", warning=FALSE, echo=FALSE, fig.align="center"}
knitr::include_graphics("figure/coho-CU-EscpSeries.png")
```


```{r coho-Subpop-timeseries, fig.cap="Escapement time series for 11 subpopulations of Interior Fraser Coho. CUs to which each subpopulation belong to are shown in Table ----", warning=FALSE, echo=FALSE, fig.align="center"}
...
For three of the five CUs, estimates of Sgen based on the IM.cap spawner recruit model were higher than those based on the IM model (Figure \@ref(fig:coho-CU-timeseries)). The North Thompson CU showed the opposite pattern, with the Sgen estimate from the IM model higher than that from the IM.cap model. The fifth CU, Fraser Canyon, has almost equal estimates for the two approaches.   
To do:
- describe how im vs. imcap model affected number of years in which each CU dropped below Sgen


The Lower North Thompson sub-population was the only sub-population that remained above the 1000 spawner threshold in all years (Figure \@ref(fig:coho-Subpop-timeseries)). All other CUs dropped below 1000 spawners in one or more years, with 2006 apparent as an especially poor year in which almost all subpopulations were below 1000 spawners. All 11 subpopulations were above 1000 spawners in 2020, which means that the SMU would be well above the LRP based on the proportional LPR approach applied to the distributional benchmark .
