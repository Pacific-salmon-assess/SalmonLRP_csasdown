...
1) Rapid multi-dimensional scanner tool
2) CU-level abundance relative to $S_{gen}$ as a lower benchmark on abundance
3) Distribution of spawning abundance relative to distributional targets developed by the IFCRT
...
| (As likely as not)  | (11,187-19,603) | (20,683-30,672) | (9,695-25,336)
+---------------------+-----------------+-----------------+-----------------+             
| 66%                 | 16,685          | 26,424          | 21,396
| (Likely)            | (12,454-20,916) | (20,478-32,369) | (13,418-29,375)
+---------------------+-----------------+-----------------+-----------------+  
| 90%                 | 19,668          | 31,969          | 30,372
| (Very likely)       | (13,752-25,584) | (21,414-42,523) | (15,711-45,033)
+---------------------+-----------------+-----------------+-----------------+
| 99%                 | 24,331          | 40,637          | 44,403
| (Virtually certain) | (13,924-34,738) | (21,288-59,936) | (15,102-73,703)
+---------------------+-----------------+-----------------+-----------------+
...
"For this case stuIdy, we consider two approaches for characterizing CU status: 
...
Proportion-based LRPs rely on CU-level benchmarks and assessments, for which data vary in quality and quantity. Our case studies demonstrate to application of proportion-based LRPs to CUs with a range of data availability including those with stock-recruitment based benchmarks, habitat-based benchmarks, and percentile-based benchmarks on spawner abundances, and CUs without relative-abundance based benchmarks where status typically relies on short and long-term trends over time, as demonstrated in application of the Rapid Multidimensional Scanner Tool. We further demonstrate the assessment of status based on proportion-based LRPs where data are missing in the <!--two case studies, Interior Fraser Coho and--> Inside South Coast Chum case study. <!--KH: edited previous sentence to remove mention of missing data scenarios for coho; we only looked at missing data for logistic in coho study -->  <!--KH, suggestion to add: "Our results from this case study highlight the importance of including data from as many CUs as possible when using the proportion-based approach, so long as individual CU-level status estimates are reliable. This recommendation is especially relevant for SMUs such as South Coast Chum in which component CUs show high variability among themselves in status estimates over time. --> Guidelines on identifying where data-rich CUs are more likely to provide status that is representative of data-deficient CUs is provided in Holt et al. (in review).
...
<!-- 
KH: could consider adding to the above paragraph with something along these lines to tie together case studies a bit more (Note: I have duplicated the first few sentences, with some minor changes):
