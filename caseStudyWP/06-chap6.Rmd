...
Proportion-based LRPs rely on CU-level benchmarks and assessments, for which data vary in quality and quantity. Our case studies demonstrate application of proportion-based LRPs to CUs with a range of data availability including those with stock-recruitment based benchmarks, habitat-based benchmarks, and percentile-based benchmarks on spawner abundances, and CUs without relative-abundance based benchmarks where status typically relies on short and long-term trends over time, as demonstrated in application of the Rapid Multidimentaional Scanner Tool. We further demonstrate the assessment of status based on proportion-based LRPs where data are missing in the <!--two case studies, Interior Fraser Coho and--> Inside South Coast Chum case study. <!--KH: edited previous sentence to remove mention of missing data scenarios for coho; we only looked at missing data for logistic in coho study -->  <!--KH, suggestion to add: "Our results from this case study highlight the importance of including data from as many CUs as possible when using the proportion-based approach, so long as individual CU-level status estimates are reliable. This recommendation is especially relevant for SMUs such as South Coast Chum in which component CUs show high variability among themselves in status estimates over time. --> Guidelines on identitying where data-rich CUs are more likely to provide status that is representative of data-deficient CUs is provided in Holt et al. (in review).
...
Third, model diagnostics do not support logistic regressions and their associated LRPs when CU-level abundances are not correlated or only weakly correlated. For these SMUs, the aggregate abundancess are not a significant indicator of individual CU statuses. The case studies shown in this paper show that logistic regression-based LRPs were estimable for interior Fraser coho (average correlation within CUS of 0.43) <!--I remember seeing the values of correlation between CU escapements(?), but I cannot find it in the current text. Where could I find those numbers? --> but not estimable for Inside South Coast Chum - Non-Fraser (average correlation within CUs of XX). Additional factors for the poor fit of the logistic regression-based LRPs for the Inside South Coast Chum - Non-Fraser study case include the presumed wide range of productivity and capacity values for the CUs within that SMU. 
