...
${obsC}_{y,i,n}$ & Observed n catches \\
...
Observed recruitment is set to the sum of observed spawners and catches. Eobserved exploitation rate is directly calculated by dividing the observed catches by th eobserved recruitment.
...
The management quantities, i.e., $Sgen$ and $Smsy$ or Spawners quantiles, can then be re-estimated based on the estimated stock recruitment parameters and the observed time series of spawners using the same procedure described in section \@ref(compute-management-quantities-and-benchmarks). The LRP study cases, however, set the management benchmarks to the normative period, and for this reason the management benchmarks are kept constant and equal to the management benchmark at the last year of the priming period.  
