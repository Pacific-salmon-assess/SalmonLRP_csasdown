...
In the priming loop, the management benchmarks are only calculated in the last two generations, i.e. if $y<nPrime-2*gen$. The management benchmarks are calculated according to three options: "stockRecruit", "percentile" and "habitat". For all three options, lower and upper benchmarks are calculated.
 ...
 If the "stockRecruit" option is used with the Ricker recruitment functions, then the lower benchmak is set to  $S_{gen}$ and the upper benchmark is set to 80\% of$S_{MSY}$. When the model with the survival covariate is used, the $\alpha$ parameter is modified to incorporate the survival component (Equation \@ref(eq:alphamb)). In order to keep the management benchmarks contant through time, the user inputted term $Survinit_{k}$ is used, which represents the long term average survival for the stock. $S_{MSY}$ is calculated following the explicit solution provided by @scheuerell_explicit_2016 (Equation  \@ref(eq:smsy)). $S_{gen}$ is estimated by solving (equation \@ref(eq:Sgen)) numerically, as described by @holt_indicators_2009. 
...
The projection phase is the phase where all scenarios and management procedures are implemented. The following description follows the order of the code implementation, therefore the subheadings in this section could be seen as pseudocode. 

#### Population Dynamics submodel

##### Specify alpha

Samsim offers the option of presenting trends in the productivity parameter, if that is true, alpha for each CU is adjusted every year by applying a linear trend to the alpha value. The slope of this trend is defined by the input option `profRegime`. A detailed description of the productivity trends is out of the scope of this report as the study cases considered in this report do not consider any scenarios with productivity changes. As alpha is held constant throughout the projections we will continue to use the time-invariant notation for the parameter: $\alpha_{k}$. 

##### Specify Management benchmarks


#### First Observation submodel
#### Management submodel
