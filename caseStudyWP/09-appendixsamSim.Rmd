...
$covMat$ & variance-covariance kxk matrix to generate recruitment deviates with CU autocorrelation\\
...
$Sgen_{y,k}$ & Number of spawners required to recover to Smsy in one generation in the absence of fishing \\
...
$o_{k}$ & shape parameter for CU-specific exploitation rate variability \\
...
$CVER$ & CV for aggregate exploitation rate variability \\
...
\begin{align}
  RY_{y,MU} &= \sum^{k \in MU} RY_{y,k}\\
  fRY_{y,MU} &= \sum^{k \in MU} fRY_{y,k}
  (\#eq:MUagg)
\end{align}

The next step is to calculate the allowable catch following a harvest control rule. Both study cases in this report use the fixed exploitation rate harvest control rule. In this option, the allowable catch is the product of fixed exploitation rates and calendar year recruits (Equation  \@ref(eq:tac)). The baseline fixed exploitation rates for Canada and the USA are defined in the input files. Currently, only Canadian exploitation rates have added variability implemented through draws from beta distributions (Equation \@ref(eq:ER) and \@ref(eq:CanERCU)). Two layers of variability are considered:  error with common mean and CVs for all CUs and error with CU-specific CVs. In the first layer, an exploitation rate value is drawn from a beta distribution with mean and CV defined by the user. In the second layer, CU-specific exploitation rates are drawn from a beta distribution using the output exploitation rate from the first layer as mean and CU-specific CV defined in the input files. The mean and CVs are transformed into shape and location parameters for the beta distribution draws (Equations \@ref(eq:ouERCU1)-\@ref(eq:ouERCU) and \@ref(eq:deER1)-\@ref(eq:deER)).

 
\begin{align}
  TAC_{y,k,i} &= ER_{y,k,i} \cdot RY_{y,MU}
  (\#eq:tac)
\end{align}
...
(\#eq:deER1)
   ...
   \vartheta &= CVER \cdot \theta_{i}  
  (\#eq:deER)
\end{align}


The TAC for Canada is further divided in two components, mixed stock fishery and single stock fisheries (Equations \@ref(eq:tacmix) and \@ref(eq:tacsingle)). Once all TACs are calculated, they are adjusted by the proportions of each CU (Equations \@ref(eq:cuppn) and \@ref(eq:tacadj)).



\begin{align}
(\#eq:tacmix)
  mixTAC_{y,k} &=  TAC_{y,k,i=Can} \cdot \psi_{y,k} \\
  singleTAC_{y,k} &= TAC_{y,k,i=Can} \cdot (1- \psi)
  (\#eq:tacsingle)
\end{align}
...
(\#eq:cuppn)
  ...
  TAC_{y,k,i} &= TAC_{y,k,i} \cdot pCU_{y,k}\\
  mixTAC_{y,k} &= mixTAC_{y,k}  \cdot pCU_{y,k}\\
  singleTAC_{y,k} &=  singleTAC_{y,k} \cdot pCU_{y,k}
  (\#eq:tacadj)
\end{align}
...
\begin{align}
  cuppn{y,k} &= RY_{y,k}/ \sum^{k}RY_{y,k}
