...
In this step, the population forecast and benchmarks are calculated. Forecast error is calculated via a scaler that is generated following a lognormal distribution with mean and variance specified by the user (Equations \@ref(eq:FctR) and \@ref(eq:Fct)). 
...
\begin{align}
  RYMU_{y,k} &= \sum^{k \in MU} $RY_{y,k}\\
  FctRYMU_{y,k} &= \sum^{k \in MU} FctRY_{y,k}
  (\#eq:MUagg)
\end{align}

The next step is to calculate the allowable catch following the inputs harvest control rule choice. Both study cases in this report use fixed exploitation rates as the harvest control rule.
