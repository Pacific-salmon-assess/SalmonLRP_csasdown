...
```{r paramtab, tab.cap="List of samSim model parameters. All quantities in this table are inputted by the user", warning=FALSE, echo=FALSE, tab.align="center"}

x <- rbind(
c("$$\\alpha_{k}$$" , "Ricker productivity parameter"),
c("$$\\beta_{k}$$" , "Ricker carrying capacity inverse")   
...
```{r modtab, tab.cap="List of samSim modeled quantities and derived variables.", warning=FALSE, echo=FALSE, fig.align="center"}

x <- rbind(
c("$$S_{y,k}$$" , "Spawners" ),
c("$$Sgen_{y,k}$$", "Number of spawners required to recover to $Smsy_{k}$ in one generation in the absence of fishing. continue just to see where it goes. I need the text to wrap.")
  )
x<-as.data.frame(x)
names(x)<-c("Notation","Definition")
csasdown::csas_table(x) %>% kable_styling(full_width = TRUE)
...
Once the age structure variability is calculated is time to calculate the recruitment deviations, given by a multivariate normal distribution reflecting the recruitment covariance among CUs. Recruitment is then calculated following the Equations \@ref(eq:Rickersimple) and \@ref(eq:Rickersurv). Following the computation of $R_{y,k}$, Samsim checks if checks whether a population has gone quasi-extinct, i.e. if Spawner abundances were below the extinction threshold for one generation.  
