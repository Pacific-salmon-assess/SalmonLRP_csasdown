...
In this section, we provide an overview of methods used to develop LRPs for our three case studies. Detailed methods specific to each case study are provided in Sections \@ref(IFCChapter) (Interior Fraser Coho), \@ref(WCVIchinookChapter) (West Coast Vancouver Island Chinook), and \@ref(ISCchumChapter) (Inner South Coast Chum, excluding Fraser). Links to data and analysis code repository used for all three case studies are in Appendix \@ref(app:github-appendix).
...
CH (14 Dec): let's leave this out (i.e., ignore my original comment)
--> 
...
The logistic regression model was implemented in TMB [@kristensenTMBAutomaticDifferentiation2016]. The model was statistically integrated, i.e., both the CU-specific lower benchmarks (S\textsubscript{gen}) and the SMU logistic regression parameters were estimated within the same statistical model. The integrated approach allowed for the propagation of uncertainty in parameter estimates from the CU level to the SMU level, resulting in more comprehensive<!--CH: 'comprehensive' might be a bit unclear. Perhaps reword to "resulting in uncertainty intervals that better captured underlying uncertainty in benchmarks"?--> uncertainty estimates for the LRPs.   
...
Statistical significance of coefficients was evaluated using the Wald test statistic, calculated from the ratio of the model coefficient<!-- LW: should we identify which coefficient, I think it is B_1?--> to the standard error of that coefficient, which is assumed to be normally distributed. Test statistics and significance were estimated within TMB [@kristensenTMBAutomaticDifferentiation2016]. 
...
An example of the derivation of an LRP from the projected curve of aggregate abundance bins versus the probability of all CUs being > their lower benchmark is shown in Figure \@ref(fig:example-projectedCurve) for the four probability levels used in our case studies (p* = 0.5, 0.66, 0.90, and 0.99). Uncertainty intervals for LRPs are not generated in this method <!--CH: changed 'but' to 'becuase'-->because <!--CH suggestion: this method does not include statistical estimation and"--> projections integrate uncertainties in all underlying parameters to identify LRPs with specified probabilities of all CUs being above LBM. In addition,<!--CH: maybe replace "In additoin" with "However"?--> LRP estimates could be presented as a range based on the $SAgg_{bin}$ bin size.
