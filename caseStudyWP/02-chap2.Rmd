...
A proportion-based LRP is simply the proportion of CUs required to be assessed as being above poor status. In all three case studies, we default to 100% of CUs being required to be above poor status when setting the LRP. In this case, the LRP acts as a trigger that is breached when one of more CUs is assessed as having poor status. Guidance on how to select the required proportion, including when it may be appropriate to consider proportions < 100%, is provided in the companion working paper *Guidelines for Defining Limit Reference Points for Pacific Salmon Stock Management Units* (Holt et al. in review). <!--CH: maybe reword the last sentence to: "Rationale for the choice of 100% of CUs as aligned with LRP is described in Holt et al. (in review) "  Holt et al. (in review) is referenced multiple times in this section; we could include the title of the companion paper at first instance and leave it at Holt et al. (in review) elsewhere?-->

We compare three different methods of assessing CU status when using proportion-based LRPs: (i) the proportion of CUs with a recent Integrated Status assessment above the red zone (e.g., @grant2017FraserSockeye2020), (ii) the proportion of CUs with a recent rapid multidimensional status assessment above red (see below for more details), and (iii) the proportion of CUs with status estimated to be below a single type of CU lower benchmark (e.g., $S_{gen}$, percentile-based benchmarks, etc.). <!-- CH: "While we recommend applying methods (i) or (ii), approach (iii) is shown for comparison purposes to demonstrate consistency between rapid multidimensional status assessments and statuses derived from abundances relative to lower benchmarks."-->


### Rapid Multidimensional Status Assessment Tool{#rapidToolMethods}<!--CH: can we keep name consistent, perhaps "Rapid Multidimensional Scanning Tool" -->
...
<!--
  CW:
  should we include a paragraph here explaining that the proportion methods (ii) multi dimensional and (iii) single metric are on a continuum and that the multi dimensional method defaults to abundance based benchmarks (i.e., Sgen) when it is available? 

Following Carrie's suggestion from the call on Friday, we may want to say that we are applying the single metric approach to illustrate the dynamics of the multi-dimensional scanner tool: i.e. both approaches will converge when a CU is data rich, and number of spawners are above the 1500 threshold. However the approaches will diverge as the information available for a CU deteriorates or its absolute abundance approaches threshold levels.   
 
 CH: I've made a brief reference to this idea above in the text, but think we can leave the details to the actual case studies.
-->


## AGGREGATE ABUNDANCE-BASED LRPS

Aggregate abundance-based LRPs represent the SMU-level abundance at which there is a sufficiently high probability that 100% of CUs <!-- CH: "(the same % as used for proportion-based LRPs)"-->will be above a single selected benchmark<!--CH: suggest change in to will have status above the red zone".-->. This definition requires a decision to be made about what represents a 'sufficiently high probability' that 100% of CUs will be above their benchmarks<!--CH: "above the red zone" instead-->. We consider four alternative probability levels for our case studies that represent a range of calibrated probability categories developed by the Intergovernmental Panel on Climate Change [@frameGuidanceNoteLead2010]: 50%, 66%, 90%, and 99%. The 50% value represents the mid-point of the "About as likely as not" category (33 - 66%), indicating that there is an equal probability that all CUs will be above their LBMs as there is that they will not. The 66% values represents the lower end of the "Likely" category (i.e., it is "Likely" that all CUs will be above their LBMs), the 90% value represents the lower end of the "Very Likely" category, and the 99% value represents the "Virtually Certain" category. A discussion of considerations for selecting the appropriate probability threshold when calculating abundance-based LRPs is included in the working paper *Guidelines for Defining Limit Reference Points for Pacific Salmon Stock Management Units* (Holt et al. in review).  For our case studies, we focus on a 50% probability threshold for sensitivity analyses, but show sensitivity to other values<!--CH: "when these can be estimated"? I couldn't estimate 90 and 99% for WCVI CK-->.  

<!--
  CW- Replace first "sensitivity" with "base case" above: 
  For our case studies, we focus on a 50% probability threshold for the base case analyses, but show sensitivity to other values.  
  -->
...
<!--CH; suggest removing this one: 4. The sample size is large. Logistic regression assumes that the sample size of the data set is large enough to draw valid conclusions from the fitted model.-->
...
<!--CH: suggest removing this one, and adding this text to discussion/caveats  **Evaluating sample size (Assumption 4)**
...
It looks the same, when compared to https://bookdown.org/egarpor/SSS2-UC3M/logreg-deviance.html, but maybe I'm missing something?

CH: Yes!! Please replace quasi- with pseudo- everywhere. My apologies!
-->

<!--CH: I've tentatively moved this text here to de-empahsize it and give it a bit of context. Can be moved elsewhere-->
...
Classification accuracy was evaluated based on the ratio of successful classifications to total number of data points in the logistic regression, also called the hit ratio. Successful classifications were the number of years when the model successfully predicted that all CUs were above their lower benchmark plus the number years when the model successfully predicted that at least one CU was below its lower benchmark.  The hit ratio tends to be biased towards unrealistically good <!-- CH: "optimistic" instead?--> classification rates when computed with the same sample used for fitting the logistic model. Therefore, we also considered an out-of-sample approach to classification accuracy, where the logistic regression was estimated iteratively removing a single data point and the occurrence of successes relative to observations were based on the model that did not contain that data point. 
...
<!--
CW:Suggest changing paragraph above to:
...
CW: I think this might be too much detail here. I suggest moving it to the appendix and the github readme. 
...
<!--
CW: the paragraphs below this point should remain in this section
-->
...
<!-- CW: I am not sure if the last sentence of the above paragraph is true. Observation error only affects the perceived population, while error in between -year ER affect the "true" (simulated) population. I suggest removing the sentence. 

  Furthermore, we were only applying a simple fixed ER harvest strategy when projecting forward, so annual variability arising from observation error would be accounted for by our specification of between-year variability in ERs.  


CH: I'm okay with removing it. While it might be true? (though CW may convince me otherwise) it is confusing and I don't think necessary.
-->
...
<!-- CH comment: in bullet above mentioned "lower benchmarks on abundances". We should also state why we didn't use multidimensional scanner tool, or state that like the logistic-regression based LRP, we relied on status on a single metric on abundances rather than the multidimensional scanner tool". Future work could consider that option, but honestly I don't think it would work so we shouldn't make promises.-->
   ...
   3. For each aggregate abundance bin, $AggS_{bin}$:
      ...
      * Summarize the realized number of year-trial combinations that fell within that bin. For example, if a projection was run for 30 years with 1000 replicates, there might be 200 year-trial combinations that had a aggregate abundance in 10,000 - 12,000 fish bin.
      ...
      <!--CH: suggest using another number than 200 for possible year-trial combinations in that bin, since the bin-size is 200, so it's a bit confusing. And I agree with CW that it should be 200 bin not 2000 bin in the example, correct?  Also, suggest revising this in the steps below.-->.
...
<!--CH comment: suggest adding a sentence that no uncertainty intervals with this approach, but it integrates all uncertainties in underlying paramters to identfiy LRP with specifice prob of all CUs being above LBM.-->
