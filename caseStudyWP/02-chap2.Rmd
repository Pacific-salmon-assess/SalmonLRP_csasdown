...
Logistic regressions require several assumptions, as listed below. Model diagnostics were applied to evaluate the extent to which those assumptions were met, the goondess-of-fit, and the classification accuracy of LRPs developed from the logsitic regression.

1. The relationship between aggregate abundance and log-odds (the logarithm of the odds of all CUs being among their lower benchmark) is linear.

2. The observations are independent of each other (i.e., residuals are not autocorrelated).

3. There are no influential outliers. 

4. The sample size is sufficiently large. Logistic regression assumes that the sample size of the dataset is large enough to draw valid conclusions from the fitted logistic regression model.


##### Evaluating assumption of linearity (Assumption 1)

A Box-Tidwell test evaluates the significance of an additional interaction term in the logistic regression, 
...
A significant interaction term indicates a relationship between aggregate abunance and log-odds that is non-linear, violating the assumption [@fox_applied_2016].

##### Evaluating independence (Assumption 2)


Deviance residuals, $d$, were then estimated for each year,
...
where $\mu$ is the predicted probability of all CUs being above their lower benchmark and $y$ is the observation (0 or 1), in a given year [@fox_applied_2016]. This equation reduces to
...
when $y=1$ (Ahmad et al. 2011).


The magnitude of autocorrelation among residuals was then estimated using R (v.4.0.4)

##### Evaluating outliers (Assumption 3)


Deviance residuals were compared to a maximum threshold of 3, beyond which they are considered to be outliers (Ahmad et al. 2011).

##### Evaluating sample size (Assumption 4)


As a rule of thumb, Peduzzi et al.(1996) suggests a minimum of 10 cases with the least frequent outcome for each explanatory variable (1 in this case). For example, if the frequency of outcomes were 0.5 and 0.5 (for 0 and 1, respectively), then the minimum sample size of at least 10 / 0.5 = 20 to avoid biases in model coefficients, but would be higher if the data were skewed, e.g., 0.7 and 0.3; mimimum sample size = 10/0.3 = 33.
