...
\item Under Canada's WSP, a composite of metrics ('multidimensional approach') are<!--'LW comment: 'is'? not sure if composite or metrics is the subject here--> used in stock assessments, including metrics on current spawner abundances, short and long-term trends in abundances, and the distribution of spawning, among others.
\item DFO has implemented the multidimensional approach for salmon assessments within the Pacific Salmon Status Scanner tool.
...
I suggest re-wording the above bullet "DFO has implemented the multidimensional approach for salmon assessments within the Pacific Salmon Status Scanner tool"
With
"DFO has recently developed the Pacific Salmon Status Scanner tool to rapidly approximate the multidimensional approach for salmon assessments"

This change is because I think 'implemented' is too strong of a word at this point.


Also - I think you could simplify kep point 2 by removing the first sentence: "LRPs can be categorized into those derived from theoretical relationships, historical patterns, or empirical analyses."  While it makes sense to include this classification in this section, we don't come back to it again, so it would be ok if readers didn't know of this.
...
\item For Pacific salmon the application of LRPs has been limited, as fisheries management is often based on alternative management approaches, including escapement goals, exploitation limits, gear restrictions, and time-area closures.
\end{itemize}
\end{tcolorbox}


<!--
KH: See my comments below related to the last bullet: "For Pacific salmon the application of LRPs has been limited, as fisheries management is often based on alternative management approaches, including escapement goals, exploitation limits, gear restrictions, and time-area closures."

May need to consider revising this bullet??

-->


For marine fish stocks in Canada, LRPs have several roles including triggering rebuilding plans under DFO's Precautionary Approach Framework @dfoFisheryDecisionMakingFramework2009 and the *Fisheries Act*, communicating conservation risks to decision makers and the public, and informing management decisions about fisheries. For example, LRPs can be used as part of measurable objectives and can also be applied as operational control points to inform operational control points which are points where management actions are triggered within harvest control rules. A conservation goal might be to avoid low biomass levels where serious harm is expected to occur which can be translated to a measurable objective to avoid spawner abundances lower than an LRP with a high probability (e.g., 95%) over a specified time period.  This objective can then inform the placement of operational control points within the harvest control rule. However, there can be several operational control points in the harvest control rule and these can be positioned to achieve a suite of biological and socioeconomic objectives beyond those associated with serious harmP, usually by a diverse range of managers, First Nations, and stakeholders. In contrast, DFO's Science Sector is responsible for identifying LRPs based only on biological considerations [@dfoScienceAdvicePrecautionary2021].<!--SUe suggested omitting these last 4  sentences, but I think this is relevant for aggregate abundance LRPs-->
