...
The estimation of the logistic regression model can be statistically integrated with the estimation of the underlying spawner-recruitment based benchmarks. This statistical integration allows uncertainties to be propagated from CU-level benchmarks to SMU-level LRPs.
...
\item For \textbf{aggregate abundance LRPs} we recommend that SMU status be provisional when SMUs contain data-deficient CUs that cannot be inferred by the data-rich CUs that are included, regardless of the status of those data-rich CUs.
\end{itemize}
\end{tcolorbox}
