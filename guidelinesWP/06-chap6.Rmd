...
Structural uncertainty in SR Model- others can be considered single vs hier, BH, hockey stock, with depensation. We've demonstrated how you can account for that type of uncertainty.

Uncertainty in percentile method because of unertainties in estimates of prod and hr to correctly classify them in framework described in Holt et al. 2018.

Uncertainty in habitat-based benchmarks, due to assumptions about productivity, meta-ansysls for stocks based on SR data that is outdated and uses a wide range of stocks that might not represent the conditions locally. <!--CH: also uncertainties about priors in the hierarchical analsysis, but this is perhaps too much deatil for now, and will leave to appendix that describes that model-->
...
- expert drive process accounts for uncertainty in underlying data an reliability of SR based benchmarks (see narratives in dfoWildSalmonPolicy2015 for IFR coho). Would also be includes in peer-reveiw of Rapid Multidimensional Scanning Tool statuses
...
As a way to adddress uncertainty in approaches (paradigms for developing LRPs), if LRP estimates align when derived using different approaches, this increases support for that value.
