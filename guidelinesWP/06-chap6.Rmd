...
\begin{itemize} 
\item Uncertainties in  \textbf{CU-level benchmarks} affect both proportional and aggregate abundance LRPs, and can arise because of:
\begin{itemize} 
...
\end{itemize}
\item Uncertainties can also arise in  \textbf{CU statuses}  due to the choice of metrics used (single or multidimensional) and their spatial scale (CU or finer, sub-populations). For both aggregate abundance LRPs, uncertainties can arise because CU-level status is based on only a single metric as a proxy for status on multiple dimensions<!--KH: suggest removing the following commented out text as it makes it sounds like the single metric approach comes from the salmon scanner--><!--as implemented in the Salmon Scanner-->. 
...
\item Uncertainties in all the candidate LRPs can arise from the exclusion of data-limited CUs from analyses when these CUs are poorly represented by the data-rich CUs that are included. We recommend caution when applying provisional LRPs that do not include all CUs.
...
<!--KH: has this next sentace about integration already been implied when we said we did it in sentence 2 of the above paragraph? -->The estimation of the logistic regression model can be statistically integrated with the estimation of the underlying spawner-recruitment based benchmarks. This statistical integration allows uncertainties to be propagated from CU-level benchmarks to SMU-level LRPs.
...
One caveat of this approach is that the LRP depends on the management procedure applied in the projections, implemented as a constant exploitation strategy for our case studies. Although management procedures for Pacific salmon often include escapement goals, fixed exploitation limits, and/or a fixed set of exploitation rates that vary with abundances, in practice these procedures<!--omit: salmon are managed through regulations on effort or exploitation rate and not allowable catches, and emergent time-series of exploitation--> can be roughly approximated with a constant exploitation rate with implementation error.<!--KH: suggestion to soften wording on previous statement from "..in practice these procedures ..." to "...we have assumed that these procedures can be roughly approximated ...."--> Other more realistic management procedures could be considered in future iterations.  Projection LRPs derived in this way cannot be used to assess status when management procedures change over time and those changes have not been evaluated in projection. 
...
\subsection*{Key Points:}
\begin{itemize} 
\item For \textbf{proportional LRPs},  we recommend SMU status be either provisional or data deficient when an SMU contains data-deficient CUs and the remaining data-rich CUs are above Red Status. An SMU's status is provisional when data-rich CUs are deemed representative of data-deficient CUs, and data-deficient when they are not. This recommendation is based on the potential for positive biases in estimated status based on the data-rich CUs alone. 
<!--KH: I made some changes to sentence structure and arrarngement in the above bullet. Is this ok? -->
\item In contrast, if any component CU has Red status, the proportional LRP of 100\% of CUs above the Red status is considered breached. This recommendation applies regardless of the presence of data-deficient CUs and whether the status data-deficient CUs can be inferred from data-rich CUs.
<!--KH: I made some changes to sentence structure and arrarngement in the above bullet. Is this ok? -->
\item For \textbf{aggregate abundance LRPs}, we recommend that SMU status be provisional when SMUs contain data-deficient CUs that can <!--KH: was cannot; changed to can --> be inferred from data-rich CUs, regardless of the status of those data-rich CUs.
...
Here's a slightly revised version of the above grey box for your consideration.  I tried to standardize sentence structure around bullets, and added extra sub-bullets for the first case.  Feel free to only use use pieces of it, or to not use any of it.

\item For \textbf{proportional LRPs} when an SMU contains data-deficient CUs and the remaining data-rich CUs are above Red Status, we recommend SMU status be either provisional or data deficient . This recommendation is based on the potential for positive biases in estimated status based on the data-rich CUs alone. 
\begin{itemize}
\item In this case, SMU status is provisional when data-rich CUs are deemed representative of data-deficient CUs
\item In this case, SMU status is data-deficient when data-rich CUs are not deemed representative of data-deficient CUs
\end{itemize}

\item For \textbf{proportional LRPs} when any component CU has Red status, we recommend SMU status be assessed as below the LRP regardless of the presence of data-deficient CUs and whether the status data-deficient CUs can be inferred from data-rich CUs.

\item For \textbf{aggregate abundance LRPs}, we recommend that SMU status be provisional when SMUs contain data-deficient CUs that can be inferred from data-rich CUs, regardless of the status of those data-rich CUs.


-->


For proportional LRPs,  we recommend SMU status be either provisional or data deficient when an SMU contains data-deficient CUs and the remaining data-rich CUs are above Red Status. This recommendation is based on the potential for positive biases in status based on the data-rich CUs alone. <!--KH: same change made as first bullet to break previous sentence into 2  -->  Provisional status can be assigned to SMUs where the data-rich CUs are representative of data-deficient CUs (reflecting high uncertainty in status), and data-deficient SMU status can be assigned where the data-rich CUs are not representative (Table \@ref(tab:GuidanceDDCUs)). The power to detect a breach of a proportional LRP is relatively weak when the sample size of the data-rich CUs is small relative to the total number of component CUs. Therefore, statuses that rely on only a small number of CUs within an SMU tend to provide more optimistic status than those that include a larger sample of CUs for proportional LRPs, as shown for Inside South Coast Chum case study. 
...
\item Adapting LRPs to include a broader scope for serious harm, including ecosystem and habitat considerations and Indigenous Knowledge.
...
Further research identifying when and how to account for time-varying parameters in assessments and management of Pacific salmon is warranted, and is currently underway within DFO. Guidance on accounting for time-varying parameters that differ among CUs within an SMU is also warranted. Time-varying stock-recruitment dynamics usually occur at the CU level, and the implications for SMU-based LRPs that contain multiple CUs are not straightforward. There are no clear guidelines on how to translate time-varying stock recruitment parameters for CUs into aggregate-level LRPs. Effects of time-varying CU recruitment dynamics on aggregate abundance- and proportion-based LRPs<!--LW comment: remove 'based'?--> will depend on the degree of synchrony among CUs, the direction of the change in recruitment parameters, and past and current stock status, among other factors. For example, in our case study on Interior Fraser River Coho Salmon, when <!--KH: changed following from previous 'consistent reductions in productivity'--> a consistently lower productivity level was introduced through an alternative stock recruitment model formulation (one that included informative priors on capacity in CU-level spawner-recruitment models), benchmarks increased <!--KH: Was previously declined, but should be increased --> for most CUs as did the aggregate abundance-based LRP for the SMU. However, in cases where productivity varies at different rates among CUs or capacity changes as well, impacts on CU-level benchmarks and SMU-level LRPs will not be easily predictable. 
