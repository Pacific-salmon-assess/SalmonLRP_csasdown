...
Uncertainties in percentile-based benchmarks can be calculated from bootstrapped confidence intervals of benchmarks accounting for autocorrelation in spawner abundances [@peacockEvaluatingConsequencesCommon2020; @holtEvaluatingBenchmarksBiological2018]. <!--CH Comment: I don't think we actually did this, but could have. Do you think it's okay to leave as is?. It would be great to get bootstrapped uncertainties in overall status by drawing from bootstrapped benchmarks and comparing against current spawner abundances. Then instead of getting a single trigger, we'd get a probability distribution that we're below the trigger. I guess we could mention this a future recommendation below...-->. Uncertainties in their implementation can arise because of uncertainties in estimates of productivity and harvest rates that are used to categorize populations to identify appropriate percentiles to use [@holtEvaluatingBenchmarksBiological2018]. 

Uncertainty in habitat-based benchmarks, due to assumptions about productivity, meta-analyse for stocks based on SR data that is outdated and uses a wide range of stocks that might not represent the conditions locally. <!--CH: also uncertainties about priors in the hierarchical analysis, but this is perhaps too much detail for now, and will leave to appendix that describes that model-->


Uncertainty in spatial scale- sens on different scales? 

Uncertainty in identification of CUs

Uncertinaty in formal WSP assessment: not repeatable, depends on experts presnet, which preusambly covers all relevant expertise. Note, this expert driven process accounts for uncertainty in underlying data an reliability of SR based benchmarks (see narratives in dfoWildSalmonPolicy2015 for IFR coho). 

Uncertainty in Scanning Tool, doesn't fit every case perfectly and doesn't capture all metrics well (e.g., productivty and distributin). Would also be includes in peer-reveiw to cpature these descrepancies. Tool will evolve as applied to more stocks. (aligned with part of principles 2 on best avialble infomation)


### Proportion-based RLPs
- describe uncertainties arising from missing limited CU-level data. (maybe not representative) 
- if CU assessments are not reliable


### Logistic-regression based LRPs

- describe uncertainties arising from missing limited CU-level data. (
- describe challenges from ppt. How do we set prob? This is an important next step to rsolve- is it only science or not?
- Also, it is not possible to assess sensitivity of LRPs to  structural assumptions about covariance in population dynamics among CU and the relationship between aggregate abundances and CU statuses, as the LRP depends only on the logistic fit to observed data

One caveat with logistic regression-based LRPs is that a single-metric dropping below the benchmark may not trigger red status over all in a multidimensional framework given high variability in spawner abundances for some CUs. In general, persistence at low levels below abundance-based benchmarks and significant declining trends are required for red status in formal, peer-reviewed WSP assessments [add refs]. 


### Projection-based LRPs
- these should be considered in development, and not used on their own without further simluation evluation.
- describe uncertainties arising from missing limited CU-level data. 
- describe challenges from ppt?


<!--CH: could add Julie's last 3/4 principles here, which are more related applying LRPs, 
e.g., 
...
As a way to address uncertainty in approaches (paradigms for developing LRPs), if LRP estimates align when derived using different approaches, this increases support for that value.
...
Simulation evaluation of management procedures [dfoManagementProcedureFramework2021] that include multiple management levers and spatial-temporal dynamics of populations and management decisions represents a gap in the current approach to informing decision for Pacific salmon, and is promising area for future research. 
...
The definition of serious harm which includes impacts to the ecosystem, associated species and a long-term loss of fishing opportunities [@dfoFisheryDecisionMakingFramework2009], depends to some extent on the time-frame being considered. Assessments that include only very recent data may miss large declines in status and ecosystem impacts that occurred historically before the advent of modern survey records. In some cases, considering a longer view from genetic, archeological, paleoecological, or traditional knowledge demonstrates that recent declines are part of much larger historical declines associated with large-scale ecosystem impacts [@eckertDivingBackTime2018; @leeDiverseKnowledgeSystems2019; @priceGeneticsCenturyOld2019; @@mckechnieArchaeologicalDataProvide2014]. <!--OMIT, unless this pgh moved to intro: The inclusion of traditional ecological knowledge is more commmon in the development of target reference points for fisheries management that identify long-term targets based on biological, social, and economic considerations [@caddyReferencePointsFisheries1995], than in LRPs. -->

While the inclusion of traditional ecological knowledge is more commmon in the development of target reference points for fisheries management [@caddyReferencePointsFisheries1995], than in LRPs, there may be value in further considering its role in identifying serious harm. @reidTwoEyedSeeingIndigenous2020 introduce the concept of Two-Eyed Seeing (Etuaptmumk in Mi’kmaw), where both indigenous and western science perspectives are valued though the process of, “learning to see from one eye with the strengths of Indigenous knowledges and ways of knowing, and from the other eye with the strengths of mainstream knowledges and ways of knowing, and to use both these eyes together, for the benefit of all”.  By understanding serious harm from both indigenous and mainstream scientific perspectives, LRPs may better reflect underlying biological processes understood through both knowledge systems.

Methods for LRPs development could further consideration of serious harm to habitat or ecosystems. In particular, it may be possible to extend the Rapid Multidimensional Scanner Tool to account for ecosystem components of serious harm, as well as divergent levels of data quantity and quality among CUs. Furthermore, the Rapid Multidimensional Scanner Tool could be extended to provide management triggers that occur prior to reaching the LRP, for example that require precautionary management and/or increased monitoring, as in @dowlingGuidelinesDevelopingFormal2015.


<!--CH add more in sentence below-->
Alternative LRPs aligned with the key principles in Section \@ref() may be developed in the future to capture a broader range of data availability, quality, and types, and dimensions of biological status.
