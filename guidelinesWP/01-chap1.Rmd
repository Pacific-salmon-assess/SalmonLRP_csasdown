...
Even before the royal assent of the revised fisheries act in 2019, many fisheries in Canada already has LRPs defined as part of their management framework. DFO policy documents such as the "Harvest Decision-Making Framework Incorporating the Precautionary Approach" (citation) and "Canada's Policy for conservation of Wild Pacific Salmon" already recommended the establishment of LRPs to protect fish stocks serious harm to managed populations. In compliance with these policies LRPs have been implemented in various marine fisheries across Canada [@marentetteSpecificationPrecautionaryApproach], examples include Pacific Herring [@kronlundSelectionRoleLimit2018], snow crab [@mullowneyBasisPrecautionaryApproach2018], lobster [@cookFrameworkAssessmentOffshore], Atlantic salmon [@chaputConsiderationsDefiningReference2015],  cite a Pacific salmon example ? sockeye?



LRPs can be based on multiple stock metrics, including abundance trends, abundance, fishing mortality, and habitat distribution [@chaputConsiderationsDefiningReference2012]. In some instances, multi dimensional approaches are also used. These approaches are particularly relevant when traditional benchmarks are not estimable or not applicable or when data availability is limited. Multi-dimensional approaches allow for consideration of multiple data sources and the explicit inclusion of expert knowledge in defining LRPs (cite snow crab, sue's tech report and the Australian papers. )

@mullowneyBasisPrecautionaryApproach2018 defines reference points for snow crab based based on a multi dimensional approach, incorporating information from CPUE estimates, egg clutches and discards. Abundance based reference points are not appropriate for crabs as the fisheries regulations ensure that females, and males below a size limit are protected from exploitation, therefore variations in catches (legal sized males) and observed biomass are not an indication of the overall health of the population. 


@portleyLimitReferencePoints2014 reviews the use of limit reference points (or their absence) for Pacific salmon stocks in the U.S. (Alaska and Pacific Northwest), Canada, Russia, and Japan. They highlight the role of LRPs in allowing for cross-jurisdictional comparisons of sustainability.

In Canada, @chaputConsiderationsDefiningReference2012 reviews the use of reference points, including LRPs, in the management of salmonids and other semelparous species. This review is focused on the stock/CU level applications of LRPs and provides a comprehensive overview of the different methods available to assess CU status and their historical applications. It also provides considerations on managing multiple stock fisheries or managing multiple stocks as unit, it highlights that mixed stock management requires more precautionary benchmarks to ensure that all stocks remain above their limit reference points.

Mention difference between previous salmon LRP work and this report: Previous work is focused on CU level LRP whereas the current work aims at providing guidance on setting LRPs for SMUs. 
