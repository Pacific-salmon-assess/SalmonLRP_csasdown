...
## Review of Reference Points for Pacific Salmon
 <!--
CW: Review of reference points or Limit reference points?
@marentetteSpecificationPrecautionaryApproach2021 is a comprehensive review of reference points for Canadian stocks up to 2018. 

Consider changing this title to Review of Reference Points in Canada with focus on Pacific Salmon


Also, I used the term "LRP" for SMU and "lower benchmark" for individual CUs-- following the terms specified in the FSP and WSP. But I am not sure if this is an obvious difference to the average reader.  Maybe it would be good to clarify this in the text? Perhaps in the intro section above? Something like:
...
The recent revision to the Fisheries Act introduced legal requirements to promote sustainability, maintain fish stocks above LRPs and implement rebuilding plans for depleted stocks in Canada [@dfoScienceAdvicePrecautionary2021]. These revisions reinforce the previously established DFO policies to manage stocks in line with the precautionary approach. The DFO policy documents "Harvest Decision-Making Framework Incorporating the Precautionary Approach" (PA Policy) [@dfoHarvestStrategyCompliant2006] and "Canada's Policy for conservation of Wild Pacific Salmon" (WSP) [@dfoCanadaPolicyConservation2005] already recommended the establishment of LRPs to protect fish stocks from serious harm. In the following paragraphs we provide a brief overview of examples and review papers on the implementation of LRPs in Canada and other jurisdictions.



 The use of LRPs, in isolation or as part of harvest control rules has been gaining considerable interest over the past decades. Reviews of the application of harvest control rules and reference points can be found in @kvamsdalHarvestControlRules2016 and @puntHarvestControlRules2010. @dowlingEmpiricalHarvestStrategies2015 reviewed the use of harvest control rules and management procedures in data-poor fisheries. In Canada, @marentetteSpecificationPrecautionaryApproach2021 reviewed the state of reference points and harvest control rules for 177 Canadian stocks of cultural, economic and/or environmental importance, excluding salmonids and transboundary stocks. They identified that 102 stocks already had identified LRPs, although the method used to estimate the LRPs varied considerably. @kronlundConsiderationsDesignRebuilding2021 also reviewed the use and definition of LRPs in the context of Canadian and international Policy. They highlight the importance of embedding LRPs into management objectives with associated tolerance (probability) statements, e.g., "stock status should be maintained above the LRP 90% of the time", as the tolerance statement is required to render the LRP operational. They also discuss the challenges associated with estimating LRPs, including difficulties in diagnosing serious harm and the potential impacts of non-stationarity.


 In terms of Salmonids, @portleyLimitReferencePoints2014 reviewed the use of LRPs and lower benchmarks (or their absence) for Pacific salmon stocks in the U.S. (Alaska and Pacific Northwest), Canada, Russia, and Japan. They highlight the role of lower benchmarks and LRPs in allowing for cross-jurisdictional comparisons of sustainability. In Canada, @chaputConsiderationsDefiningReference2012 reviewed the use of reference points, including lower benchmarks and LRPs, in the management of salmonids and other semelparous species. The review was focused on the applications of lower benchmarks at the Conservation Unit (CU) level, providing a comprehensive overview of the different methods available to assess CU status and their historical applications. @chaputConsiderationsDefiningReference2012 also provide considerations on managing multiple stocks as unit, e.g. multiple CUs as an SMU, highlighting that mixed stock management requires more precautionary benchmarks to ensure that all sub units remain above their lower benchmarks.
...
For Pacific salmon, the WSP establishes that management should occur at the Conservation Unit (CU) level. CUs are defined as sufficiently isolated groups that, if lost, are very unlikely to be recolonized naturally within an acceptable time frame [dfoCanadaPolicyConservation2005]. The WSP is aligned with the precautionary approach in establishing three biological status zones for each CU, which are delimited by lower and higher benchmarks. The recommended lower benchmark is $Sgen$, i.e., the spawner abundances that will result in recovery to $S_{MSY}$ in one generation under equilibrium conditions, although alternative quantities can be used in the absence of the required data to estimate $Sgen$ [@holtEvaluationBenchmarksConservation2009]. The establishment of lower benchmarks are part of the WSP status assessments. To date, these assessments have been conducted for many Pacific Salmon CUs, including Fraser River sockeye salmon [@bradfordStatusCultusLake2010; @grantEvaluationUncertaintyFraser2011], coho salmon [@deckerAssessmentInteriorFraser2014; @dfoWildSalmonPolicy2015; @nobleHabitatbasedEscapementBenchmarks2015] , chum salmon [@godboutStockStatusWild2004], and chinook salmon [@dfoIntegratedBiologicalStatus2016].
