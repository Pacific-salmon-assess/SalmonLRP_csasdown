...
<!-- CH comment: see Ron's email from 25 Oct about similarities among CUs within SMUs, in some cases but not all. see bottom of this file-->
...
<!-- Ron K.'s email from 25 Oct
•	The current 69 SMUs are an FSP stock based evolution of the 53 Management Units (MUs) covered in the IFMP. National Sustainability Surveys were completed for 16 of these MUs.   
•	To my knowledge, SMUs under the FSPs has not been defined beyond what you’ve indicated, “ a group of one or more WSP CUs of the same species that are managed together.” Since most fisheries are in mixed stock areas this means that most SMUs include a wide range of CUs that co-migrate and for which there is there is also a wide range of available data. I would expect that as fisheries become more precautionary and targeted in future that SMUs might contain fewer CUs.  For example, the Skeena sockeye SMU currently has 33 CUs but in future, it could be sub-divided into run timing and enhanced/wild components. 
•	Even in the current list though there are some SMUs that are grouped by age class (Fraser chinook), run timing (Fraser sockeye) and odd-even life cycle (Fraser pink, ECVI/Mnld pink). Also, I suspect that as COSEWIC identifies DUs for sockeye, pink and chum that are more closely linked to CUs that our definition of SMUs might change in future. 
•	As for developing LRPs for SMUs that contain some data limited CUs, I wonder if the indicator stock approach that we use in chinook and coho assessment and management might also work here. Regardless of how SMUs are defined and constructed there will always be limitations to the data that can be collected with available resources. I think that an LRP approach that recognizes and accommodates this variability in data will be very useful and should be considered. Formalizing this type of approach could also help in prioritizing data collection for work planning purposes.
•	Finally, as far as data limited SMUs are concerned, there will be some SMUs with only qualitative indications of abundance or status as indicated in the annual Outlook document. These SMUs should probably not be prescribed in regulation unless additional data are collected, but should be managed in a very precautionary manner, e.g. longer term fishery closures.

Diana's email 26 Oc
•	At a first cut, we attempted to align SMUs to MUs that are already defined in place for implementation of the management and assessment frameworks implemented for the PST.  (We do not want to create multiple scales of organization that are not well aligned for any purpose – is inefficient and confusing (e.g. where CUs and DUs are not aligned).)  
•	While some get caught up in the CU, SMU scale issues, it would be unfair to say SMUs have no biological basis.  Even within freshwater basins, for the most part they are geographic clusters of CUs.  (Fraser exceptions).   They are ‘managed together’ as Ron describes below BECAUSE they co-migrate.  Of course, from an ecosystem-based management point of view, ‘management’ is bigger than harvest impacts, but i) co-migration patterns would presumably result in similar marine risks/pressures and ii) SMU-scale aggregation within freshwater basins is also related to some extent to old ‘production areas’ where f/w risks/pressures may be fairly similar across CUs within the SMU at least at a high level.
•	I agree with Ron that aggregation issues related to how to define LRPs or assess SMU status is complicated by data and there needs to be some flexibility with these evaluations.   I suppose that is a subject of the work you are up to.  i.e. How well do CU statuses correlate within an SMU?
