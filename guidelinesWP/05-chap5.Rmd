...
### When are aggregate abundance LRP methods appropriate?

-	Based on experience applying LRPs to case studies, develop recommendations on when aggregate abundance LRPs are appropriate or not (and when proportions of CUs $>$ red status, CU-level statuses without aggregation, or multi-dimensional approaches should be considered instead). Do these recommendations depend on the number of CUs (e.g., what if the SMU only has 1 or 2 CUs)?
...
- Are projected LRPs always preferred when possible?  Under what circumstances are empirical LRPs warranted?
...
## What proportional threshold value should be used for aggregate-abundance LRPs?
...
## Selection of CU-level benchmarks to use as a basis for LRP estimation (Placeholder) 
...
-	When concerns exist about changes or trends in productivity or capacity, model scenarios that allow for such trends (e.g.,  Kalman filter on productivity parameters) can be explicitly incorporated into LRP formulation. Provide example from a case study.
