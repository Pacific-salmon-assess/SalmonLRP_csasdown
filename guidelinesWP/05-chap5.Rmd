...
The first step is to compile CU-level data to support biological status assessments as described under the Wild Salmon Policy [@holtIndicatorsStatusBenchmarks2009a; @holtEvaluatingBenchmarksBiological2018], where those data exist. These data include (but are not limited to) time-series of spawner abundances and biological benchmarks on spawner abundances.  This step generally includes aggregating fine-spatial scale data on stocks or spawning sites to the CU level. 

In some cases, finer spatial scales (e.g., watersheds within CUs) may be considered to delineate serious harm to the SMU. Although CUs are the population unit of assessment and conservation required under the WSP, distribution of spawning within CUs, for example among watersheds, can be an important component of status [@holtIndicatorsStatusBenchmarks2009a] and these finer scales can be considered. 

Distribution is a commonly cited component of salmon viablity and recovery [@mcelhanyViableSalmonidPopulations2010]. When applied to the identification of recovery targets for Pacific salmon, @bradfordReviewBiologicalPrinciples2004 defined the relevant scale of distribution as spatial units that are demographically independent such that the  population dynamics of one unit are unlikely to affect the dynamics of another, but that genetic exchange is expected to occur at greater rates than observed among populations (CUs), and would likely exceed 10 effective migrants/generation. When applying these criteria to Interior Fraser Coho, they defined subpopulations on the basis of large watersheds or lakes, or partial barriers to migration. 

We recommend that the decision on spatial scale of conservation for biological assessments be part of the peer-review process for SMU assessments, and be based on biological principles of conservation and viability (e.g., as reviewed in @bradfordReviewBiologicalPrinciples2004). 

<!--CH: this next pgh needs to be reworked and shortened given the section in the Intro. Same for the section in the WCVK CK case study-->
...
CW: I think it may be beneficial to cite these. Especially Brown et al. as it has been published. -- I am not sure bout what the others are?
-->
